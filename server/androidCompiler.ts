  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      gradle.on("close", async (code) => {
        ...
        try {
          // Upload APK to S3
          addLog(`📂 Lendo arquivo APK (${apkSize} MB)...`);
          const apkBuffer = fs.readFileSync(apkPath);
          addLog(`✅ Arquivo lido com sucesso`);
          
          const timestamp = Date.now();
          const fileKey = `apks/${timestamp}-${apkFiles[0]}`;
          addLog(`🔑 Chave do arquivo: ${fileKey}`);
          
          addLog(`📤 Iniciando upload para S3...`);
          ...
          const errorStack = uploadError instanceof Error ? uploadError.stack : "";
          addLog(`❌ Erro ao fazer upload: ${errorMsg}`);
          if (errorStack) {
            addLog(`📋 Stack trace: ${errorStack}`);
          }
          
          // Fallback: save APK locally and provide download link
          addLog(`💡 Tentando salvar APK localmente...`);
          const localPath = `/tmp/${apkFiles[0]}`;
          fs.copyFileSync(apkPath, localPath);
          addLog(`✅ APK salvo em: ${localPath}`);
          
          resolve({
            success: true,
            apkPath: localPath,
            apkUrl: localPath,
            apkFileKey: apkFiles[0]!,
            logs
          });
        }
      });
